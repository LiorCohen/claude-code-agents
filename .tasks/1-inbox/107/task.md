...
| 4 | The template package.json is missing deps documented in the frontend stack (TanStack Router, Zustand, TanStack Table, TanStack Form). Should we fix this? | Yes. Fix all gaps as part of this task. |
...
| 11 | The scaffold has `api/`, `utils/`, `models/`, `stores/` directories that overlap with other patterns. Keep or remove? | Remove all four. `lib/utils.ts` covers utilities. Hooks manage Zustand stores internally. API clients live in `services/`. |
